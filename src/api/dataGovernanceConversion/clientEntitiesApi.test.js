...
import clientEntitiesApi from './clientEntitiesApi';
...
describe('ClientEntitiesApi', () => {
  ...
  describe('get', () => {
    ...
    it('invokes api methods with correct parameters', async () => {
      // * ARRANGE
      const expectedResourceName = 'clientEntities';
      const expectedParams = { apiVersion: 1, ...defaultParams };

      // * ACT
      await clientEntitiesApi.get(defaultParams);
    ...
    it('returns results from the api', async () => {
      const actual = await clientEntitiesApi.get(defaultParams);
