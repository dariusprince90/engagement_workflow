...
import taxTypesApi from './taxTypesApi';
...
describe('TaxTypesApi', () => {
  ...
  describe('get', () => {
    ...
    it('invokes api methods with correct parameters', async () => {
      // * ARRANGE
      const expectedResourceName = 'taxTypes';
      const expectedParams = { apiVersion: 1, ...defaultParams };

      // * ACT
      await taxTypesApi.get(defaultParams);
    ...
    it('returns results from the api', async () => {
      const actual = await taxTypesApi.get(defaultParams);
