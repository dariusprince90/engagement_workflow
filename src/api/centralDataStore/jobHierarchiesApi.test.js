...
import jobHierarchiesApi from './jobHierarchiesApi';
...
describe('JobHierarchiesApi', () => {
  ...
  describe('get', () => {
    ...
    it('invokes api methods with correct parameters', async () => {
      // * ARRANGE
      const expectedResourceName = 'jobHierarchies';
      const expectedParams = { apiVersion: 1, ...defaultParams };

      // * ACT
      await jobHierarchiesApi.get(defaultParams);
    ...
    it('returns results from the api', async () => {
      const actual = await jobHierarchiesApi.get(defaultParams);
