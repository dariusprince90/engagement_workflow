...
import jobCategoryRolesApi from './jobCategoryRolesApi';
...
describe('JobCategoryRolesApi', () => {
  ...
  describe('get', () => {
    ...
    it('invokes api methods with correct parameters', async () => {
      // * ARRANGE
      const expectedResourceName = 'jobCategoryRoles';
      const expectedParams = { apiVersion: 1, ...defaultParams };

      // * ACT
      await jobCategoryRolesApi.get(defaultParams);
    ...
    it('returns results from the api', async () => {
      const actual = await jobCategoryRolesApi.get(defaultParams);
