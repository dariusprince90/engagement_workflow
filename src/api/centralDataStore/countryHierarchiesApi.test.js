...
import countryHierarchiesApi from './countryHierarchiesApi';
...
describe('CountryHierarchiesApi', () => {
  // **********************************************************************
  // * setup
  ...
  describe('get', () => {
    ...
    it('invokes api methods with correct parameters', async () => {
      // * ARRANGE
      const expectedResourceName = 'countryHierarchies';
      const expectedParams = { apiVersion: 1, ...defaultParams };

      // * ACT
      await countryHierarchiesApi.get(defaultParams);
    ...
    it('invokes api methods with correct parameters when no params are passed', async () => {
      ...
      // * ACT
      await countryHierarchiesApi.get();
    ...
    it('returns results from the api', async () => {
      const actual = await countryHierarchiesApi.get(defaultParams);
