...
import ownershipTypesApi from './ownershipTypesApi';
...
describe('OwnershipTypesApi', () => {
  ...
  describe('get', () => {
    ...
    it('invokes api methods with correct parameters', async () => {
      // * ARRANGE
      const expectedResourceName = 'ownershipTypes';
      const expectedParams = { apiVersion: 1, ...defaultParams };

      // * ACT
      await ownershipTypesApi.get(defaultParams);
    ...
    it('returns results from the api', async () => {
      const actual = await ownershipTypesApi.get(defaultParams);
