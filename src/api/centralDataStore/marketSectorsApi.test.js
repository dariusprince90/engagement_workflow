...
import marketSectorsApi from './marketSectorsApi';
...
describe('MarketSectorsApi', () => {
  ...
  describe('get', () => {
    ...
    it('invokes api methods with correct parameters', async () => {
      // * ARRANGE
      const expectedResourceName = 'marketSectors';
      const expectedParams = { apiVersion: 1, ...defaultParams };

      // * ACT
      await marketSectorsApi.get(defaultParams);
    ...
    it('returns results from the api', async () => {
      const actual = await marketSectorsApi.get(defaultParams);
