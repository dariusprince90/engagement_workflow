...
import regionHierarchiesApi from './regionHierarchiesApi';
...
describe('RegionHierarchiesApi', () => {
  ...
  describe('get', () => {
    ...
    it('invokes api methods with correct parameters', async () => {
      // * ARRANGE
      const expectedResourceName = 'regionHierarchies';
      const expectedParams = { apiVersion: 1, ...defaultParams };

      // * ACT
      await regionHierarchiesApi.get(defaultParams);
    ...
    it('invokes api methods with correct parameters when no params are passed', async () => {
      ...
      // * ACT
      await regionHierarchiesApi.get();
    ...
    it('returns results from the api', async () => {
      const actual = await regionHierarchiesApi.get(defaultParams);
