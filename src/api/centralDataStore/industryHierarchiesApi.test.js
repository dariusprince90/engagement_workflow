...
import industryHierarchiesApi from './industryHierarchiesApi';
...
describe('IndustryHierarchiesApi', () => {
  // **********************************************************************
  // * setup
  ...
  describe('get', () => {
    ...
    it('invokes api methods with correct parameters', async () => {
      // * ARRANGE
      const expectedResourceName = 'industryHierarchies';
      const expectedParams = { apiVersion: 1, ...defaultParams };

      // * ACT
      await industryHierarchiesApi.get(defaultParams);
    ...
    it('invokes api methods with correct parameters when no params are passed', async () => {
      ...
      // * ACT
      await industryHierarchiesApi.get();
    ...
    it('returns results from the api', async () => {
      const actual = await industryHierarchiesApi.get(defaultParams);
