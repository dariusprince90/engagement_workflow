...
import clientTaxTypesApi from './clientTaxTypesApi';
...
describe('ClientTaxTypesApi', () => {
  ...
  describe('get', () => {
    ...
    it('invokes api methods with correct parameters', async () => {
      // * ARRANGE
      const expectedResourceName = 'clientTaxTypes';
      const expectedParams = { apiVersion: 1, ...defaultParams };

      // * ACT
      await clientTaxTypesApi.get(defaultParams);
    ...
    it('returns results from the api', async () => {
      const actual = await clientTaxTypesApi.get(defaultParams);
