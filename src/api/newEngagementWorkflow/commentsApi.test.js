...
describe('CommentsApi', () => {
  ...
  describe('get', () => {
    ...
    it('invokes api methods with correct parameters', async () => {
      // * ARRANGE
      const expectedResourceName = 'comments';
      const expectedParams = { apiVersion: 1, ...defaultParams };

      // * ACT
      await commentsApi.get(defaultParams);
    ...
    it('returns results from the api', async () => {
      const actual = await commentsApi.get(defaultParams);
  ...
  describe('getComment', () => {
    ...
    it('invokes api methods with correct parameters', async () => {
      ...
      // * ACT
      await commentsApi.getComment(id, fields);
    ...
    it('returns results from the api', async () => {
      const actual = await commentsApi.getComment(id, fields);
  ...
  describe('post', () => {
    ...
    it('returns results from the api', async () => {
      const data = { [faker.random.alpha(5)]: faker.random.alphaNumeric(10) };
      const actual = await commentsApi.post(data);
      expect(actual).toBe(fakeResource);
    });
  });
});
