...
import clientSearchTypesApi from './clientSearchTypesApi';
...
describe('ClientSearchTypesApi', () => {
  ...
  describe('get', () => {
    ...
    it('invokes api methods with correct parameters', async () => {
      // * ARRANGE
      const expectedResourceName = 'clientSearchTypes';
      const expectedParams = { apiVersion: 1, ...defaultParams };

      // * ACT
      await clientSearchTypesApi.get(defaultParams);
    ...
    it('invokes api methods with correct parameters when no params are passed', async () => {
      ...
      // * ACT
      await clientSearchTypesApi.get();
    ...
    it('returns results from the api', async () => {
      const actual = await clientSearchTypesApi.get(defaultParams);
