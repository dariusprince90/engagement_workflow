...
import naturesOfServicesApi from './naturesOfServicesApi';
...
describe('NaturesOfServicesApi', () => {
  ...
  describe('get', () => {
    ...
    it('invokes api methods with correct parameters', async () => {
      // * ARRANGE
      const expectedResourceName = 'naturesOfServices';
      const expectedParams = { apiVersion: 1, ...defaultParams };

      // * ACT
      await naturesOfServicesApi.get(defaultParams);
    ...
    it('invokes api methods with correct parameters when no params are passed', async () => {
      ...
      // * ACT
      await naturesOfServicesApi.get();
    ...
    it('returns results from the api', async () => {
      const actual = await naturesOfServicesApi.get(defaultParams);
