...
import incompatibleNaturesOfServicesApi from './incompatibleNaturesOfServicesApi';
...
describe('IncompatibleNaturesOfServicesApi', () => {
  // **********************************************************************
  // * setup
  ...
  describe('get', () => {
    ...
    it('invokes api methods with correct parameters', async () => {
      // * ARRANGE
      const expectedResourceName = 'incompatibleNaturesOfServices';
      const expectedParams = { apiVersion: 1, ...defaultParams };

      // * ACT
      await incompatibleNaturesOfServicesApi.get(defaultParams);
    ...
    it('invokes api methods with correct parameters when no params are passed', async () => {
      ...
      // * ACT
      await incompatibleNaturesOfServicesApi.get();
    ...
    it('returns results from the api', async () => {
      const actual = await incompatibleNaturesOfServicesApi.get(defaultParams);
