...
import subjectToSecOrGaoRulesApi from './subjectToSecOrGaoRulesApi';
...
describe('SubjectToSecOrGaoRulesApi', () => {
  // **********************************************************************
  // * setup
  ...
  describe('get', () => {
    ...
    it('invokes api methods with correct parameters', async () => {
      // * ARRANGE
      const expectedResourceName = 'subjectToSecOrGaoRules';
      const expectedParams = { apiVersion: 1, ...defaultParams };

      // * ACT
      await subjectToSecOrGaoRulesApi.get(defaultParams);
    ...
    it('invokes api methods with correct parameters when no params are passed', async () => {
      ...
      // * ACT
      await subjectToSecOrGaoRulesApi.get();
    ...
    it('returns results from the api', async () => {
      const actual = await subjectToSecOrGaoRulesApi.get(defaultParams);
