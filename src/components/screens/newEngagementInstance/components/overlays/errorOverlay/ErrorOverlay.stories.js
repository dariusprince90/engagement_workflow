...
export const state = { metadata: { error: { message: '', traceId: '' } } };
...
const mockSlice = createSlice({
  ...
  reducers: {
    updateError: (state, action) => {
      ...
      state.metadata.error.message = message;
      state.metadata.error.traceId = traceId;
    }
  }
});

const { updateError } = mockSlice.actions;
...
export default {
  title: 'NEI Screen/Components/Overlays/ErrorOverlay',
  ...
  parameters: {
    docs: {
      description: { component: 'A full-screen overlay which displays error information' }
    }
  },
...
const Template = (args) => {
  ...
  useEffect(() => {
    const payload = { message: args.errorMessage, traceId: args.traceId };
    dispatch(updateError(payload));
  }, [args.errorMessage, args.traceId, dispatch]);
